...
public class GetInfoListTool {
    ...
    public static int getDistance(LatLonPoint startPoint, LatLonPoint endPoint, Context context) {
        Route.FromAndTo fromAndTo = new Route.FromAndTo(startPoint,
                endPoint);
        List<Route> routeResult = null;
        Route route;
        int distance = 0;
        try {
            routeResult = Route.calculateRoute((Activity) context,
